...
const projects = [
  ...
  { category: "video", title: "Intramurals and Scimath 2023", desc: "A short video edit highlighting the Intramurals and Scimath Activity.", img: film1, fallback: "https://images.unsplash.com/photo-1485846234645-a62644f84728?auto=format&fit=crop&q=80&w=800", link: "https://youtu.be/Z6AojJ9b_Ho?si=L_8GBJHMmzp-v4Y3", linkText: "Watch Video", isVideo: true },
...
export default function App() {
  ...
  const renderAllProjects = () => (
    <div className="w-full max-w-5xl mx-auto px-5 py-12 fade-up">
      <div className="flex items-center gap-4 mb-8">
        <button onClick={() => navigateTo('home')} className={`flex items-center gap-2 font-medium text-sm transition-colors ${theme === 'dark' ? 'text-zinc-400 hover:text-white' : 'text-zinc-500 hover:text-zinc-900'}`}>
        ...
        <h1 className={`text-xl font-extrabold tracking-tight ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>All Projects</h1>
      </div>
      ...
      <div className={`flex gap-4 mb-6 border-b pb-2 flex-wrap ${theme === 'dark' ? 'border-zinc-800' : 'border-zinc-200'}`}>
        {['all', 'uiux', 'web', 'video'].map(filter => (
          <button 
            ...
            className={`text-xs font-bold uppercase tracking-wide pb-2 border-b-2 transition-colors ${activeProjectFilter === filter ? (theme === 'dark' ? 'text-white border-white' : 'text-zinc-900 border-zinc-900') : (theme === 'dark' ? 'text-zinc-400 border-transparent hover:text-white' : 'text-zinc-500 border-transparent hover:text-zinc-900')}`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {projects.filter(p => activeProjectFilter === 'all' || p.category === activeProjectFilter).map((proj, idx) => (
          <a 
            ...
            target="_blank" 
            rel="noreferrer"
            className={`flex flex-col p-5 rounded-lg border transition-all no-underline group ${theme === 'dark' ? 'border-zinc-800 bg-zinc-950 hover:bg-zinc-900' : 'border-zinc-200 bg-white hover:bg-zinc-50'}`}
          >
            <h3 className={`text-base font-bold mb-1.5 tracking-tight transition-colors group-hover:opacity-80 ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>{proj.title}</h3>
            <p className={`text-sm leading-relaxed mb-5 ${theme === 'dark' ? 'text-zinc-400' : 'text-zinc-500'}`}>{proj.desc}</p>
            <div className="mt-auto">
              <span className={`inline-block px-2 py-0.5 font-mono text-xs font-medium rounded border ${theme === 'dark' ? 'bg-black text-white border-zinc-800' : 'bg-zinc-50 text-zinc-900 border-zinc-200'}`}>
  ...
  const renderAllCerts = () => (
    <div className="w-full max-w-5xl mx-auto px-5 py-12 fade-up">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className={`text-xl font-extrabold tracking-tight ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>All Certifications</h1>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {certifications.map((cert, index) => (
          <div 
            key={index} 
            className={`flex flex-col p-5 border rounded-lg cursor-pointer transition-all ${theme === 'dark' ? 'border-zinc-800 bg-zinc-950 hover:bg-zinc-900' : 'border-zinc-200 bg-white hover:bg-zinc-50'}`}
            onClick={() => openModal(cert.image)}
          >
            <h3 className={`text-base font-bold mb-1 tracking-tight ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>{cert.title}</h3>
            <p className={`text-sm font-medium ${theme === 'dark' ? 'text-zinc-400' : 'text-zinc-500'}`}>{cert.issuer}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  const renderAllGallery = () => (
    <div className="w-full max-w-5xl mx-auto px-5 py-12 fade-up">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className={`text-xl font-extrabold tracking-tight ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>All Photos</h1>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {galleryImages.map((image, idx) => (
          <div 
            key={idx} 
            className={`flex aspect-square rounded-lg overflow-hidden hover:opacity-80 transition-opacity cursor-pointer border ${theme === 'dark' ? 'border-zinc-800 bg-zinc-950' : 'border-zinc-200 bg-white'}`}
  ...
  return (
    <div className={`min-h-screen w-full transition-colors duration-300 font-sans pb-16 ${theme === 'dark' ? 'bg-black text-white selection:bg-white selection:text-black' : 'bg-zinc-50 text-zinc-900 selection:bg-zinc-900 selection:text-white'}`}>
      ...
      {/* MODAL */}
      <div 
        className={`fixed inset-0 w-full h-full backdrop-blur-md z-50 flex justify-center items-center transition-all duration-300 ${isModalOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} ${theme === 'dark' ? 'bg-black/90' : 'bg-black/70'}`}
        onClick={(e) => e.target === e.currentTarget && closeModal()}
      >
        <div className={`relative w-11/12 max-w-5xl max-h-[90vh] rounded-lg overflow-hidden shadow-2xl transition-transform duration-300 ${isModalOpen ? 'scale-100' : 'scale-95'}`}>
          <button 
            onClick={closeModal} 
      ...
      <header className="w-full z-40 bg-transparent pt-6 pb-2">
        <div className="w-full max-w-5xl mx-auto px-5 flex justify-between items-center">
          ...
          <button onClick={toggleTheme} aria-label="Toggle Theme" className={`border rounded-full p-2 transition-colors ${theme === 'dark' ? 'bg-zinc-950 border-zinc-800 text-zinc-400 hover:text-white' : 'bg-white border-zinc-200 text-zinc-500 hover:text-zinc-900'}`}>
            {theme === 'dark' ? (
        ...
        <main className="w-full max-w-5xl mx-auto px-5 fade-up">
            ...
            <div className="flex items-center gap-5">
              ...
              <div>
                <h1 className={`text-xl md:text-2xl font-extrabold tracking-tight leading-none mb-1.5 flex items-center gap-2 ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>
                  Lucas Eguna
                  <svg className="w-4 h-4 shrink-0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 22C6.477 22 2 17.523 2 12S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z" fill="#1d9bf0"/><path d="M10 15.172l-3.536-3.536 1.414-1.414L10 12.344l5.656-5.656 1.414 1.414L10 15.172z" fill="#ffffff"/></svg>
                </h1>
                ...
                <p className={`text-[0.8rem] md:text-[0.85rem] font-bold tracking-tight ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>UI/UX Designer \ Full Stack Developer \ Video Editor</p>
              </div>
            </div>
            
            <div className="flex gap-3 w-full md:w-auto mt-2 md:mt-0">
              <a href={resumeDoc} target="_blank" rel="noopener noreferrer" className={`px-4 py-2 rounded-md font-bold text-[0.75rem] flex items-center justify-center gap-1.5 transition-opacity flex-1 md:flex-none border border-transparent ${theme === 'dark' ? 'bg-white text-zinc-900 hover:opacity-90' : 'bg-zinc-900 text-white hover:opacity-90'}`}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-3.5 h-3.5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                Resume
              </a>
              <a href="mailto:egunalucasandrei@gmail.com" target="_blank" rel="noopener noreferrer" className={`bg-transparent border px-4 py-2 rounded-md font-bold text-[0.75rem] flex items-center justify-center gap-1.5 transition-colors flex-1 md:flex-none ${theme === 'dark' ? 'border-zinc-800 text-white hover:bg-zinc-900' : 'border-zinc-200 text-zinc-900 hover:bg-zinc-100'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-start">
            ...
            <div className="md:col-span-7 flex flex-col gap-5 w-full">
              ...
              <div className={`border rounded-xl p-5 md:p-6 ${theme === 'dark' ? 'bg-zinc-950 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                <h2 className={`text-[0.9rem] font-extrabold mb-4 tracking-tight ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>Tech Stack & Tools</h2>
                <div className="flex flex-col gap-5">
                  ...
                  <div>
                    <h3 className={`text-[0.65rem] font-bold mb-2 uppercase tracking-wider ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>Backend</h3>
                    <div className="flex flex-wrap gap-2">
                  ...
                  <div>
                    <h3 className={`text-[0.65rem] font-bold mb-2 uppercase tracking-wider ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>Developer Tools</h3>
                    <div className="flex flex-wrap gap-2">
                      {['GitHub', 'VSCode', 'Vercel', 'Netlify', 'Capcut', 'Figma', 'Canva'].map(tech => (
                        <span key={tech} className={`text-[0.65rem] font-medium px-2 py-0.5 rounded border ${theme === 'dark' ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-zinc-50 border-zinc-200 text-zinc-900'}`}>{tech}</span>
                      ))}
                    </div>
                  </div>
              ...
              <div className={`border rounded-xl p-5 md:p-6 ${theme === 'dark' ? 'bg-zinc-950 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                <div className="flex justify-between items-center mb-4">
                  <h2 className={`text-[0.9rem] font-extrabold tracking-tight ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>Recent Projects</h2>
                  <button onClick={() => navigateTo('projects')} className={`bg-transparent border-none text-[0.65rem] font-bold cursor-pointer inline-flex items-center gap-1 transition-colors group ${theme === 'dark' ? 'text-zinc-400 hover:text-white' : 'text-zinc-500 hover:text-zinc-900'}`}>
                    View All
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {projects.slice(0, 4).map((proj, idx) => (
                    <div 
                      key={idx} 
                      className={`flex flex-col p-4 rounded-md border transition-all group cursor-pointer ${theme === 'dark' ? 'border-transparent hover:border-zinc-800 hover:bg-zinc-900' : 'border-transparent hover:border-zinc-200 hover:bg-zinc-50'}`}
                      onClick={() => window.open(proj.link, '_blank')}
                    >
                      <h3 className={`text-[0.8rem] font-bold mb-1 tracking-tight transition-colors line-clamp-1 ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>{proj.title}</h3>
                      <p className={`text-[0.7rem] leading-relaxed mb-3 line-clamp-2 ${theme === 'dark' ? 'text-zinc-400' : 'text-zinc-500'}`}>{proj.desc}</p>
                      <div className="mt-auto">
                        <span className={`inline-block px-2 py-0.5 font-mono text-[0.6rem] font-medium rounded border ${theme === 'dark' ? 'bg-black text-white border-zinc-800' : 'bg-zinc-50 text-zinc-900 border-zinc-200'}`}>{getDomainText(proj.linkText, proj.link)}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className={`border rounded-xl p-5 md:p-6 ${theme === 'dark' ? 'bg-zinc-950 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                <div className="flex justify-between items-center mb-4">
                  <h2 className={`text-[0.9rem] font-extrabold tracking-tight ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>Recent Certifications</h2>
                  <button onClick={() => navigateTo('certs')} className={`bg-transparent border-none text-[0.65rem] font-bold cursor-pointer inline-flex items-center gap-1 transition-colors group ${theme === 'dark' ? 'text-zinc-400 hover:text-white' : 'text-zinc-500 hover:text-zinc-900'}`}>
                    View All
                ...
                <div className={`flex flex-col border rounded-md overflow-hidden ${theme === 'dark' ? 'border-zinc-800 bg-black' : 'border-zinc-200 bg-zinc-50'}`}>
                  {certifications.slice(0, 3).map((cert, index) => (
                    <div 
                      key={index} 
                      className={`flex flex-col py-2 px-3 border-b last:border-0 cursor-pointer transition-colors ${theme === 'dark' ? 'border-zinc-800 hover:bg-zinc-900' : 'border-zinc-200 hover:bg-zinc-100'}`}
                      onClick={() => openModal(cert.image)}
                    >
                      <h3 className={`text-[0.75rem] font-bold tracking-tight mb-0.5 ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>{cert.title}</h3>
                      <p className={`text-[0.65rem] font-medium ${theme === 'dark' ? 'text-zinc-400' : 'text-zinc-500'}`}>{cert.issuer}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* LET'S CONNECT */}
              <div className={`border rounded-xl p-5 flex flex-col sm:flex-row gap-5 justify-between items-start sm:items-center ${theme === 'dark' ? 'bg-zinc-950 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                <div>
                  <h3 className={`text-[0.75rem] font-bold mb-2.5 tracking-tight ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>Let's Connect</h3>
                  <div className="flex gap-3">
                    <a href="https://github.com/egunalucasandrei-droid" target="_blank" rel="noreferrer" className={`transition-colors ${theme === 'dark' ? 'text-zinc-400 hover:text-white' : 'text-zinc-500 hover:text-zinc-900'}`}><svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"></path></svg></a>
                    <a href="https://www.linkedin.com/in/lucas-andrei-eguna-521b7530a/" target="_blank" rel="noreferrer" className={`transition-colors ${theme === 'dark' ? 'text-zinc-400 hover:text-white' : 'text-zinc-500 hover:text-zinc-900'}`}><svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"></path><rect x="2" y="9" width="4" height="12"></rect><circle cx="4" cy="4" r="2"></circle></svg></a>
                    <a href="https://www.youtube.com/@LucasAndrei-Creatives" target="_blank" rel="noreferrer" className={`transition-colors ${theme === 'dark' ? 'text-zinc-400 hover:text-white' : 'text-zinc-500 hover:text-zinc-900'}`}><svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22.54 6.42a2.78 2.78 0 0 0-1.94-2C18.88 4 12 4 12 4s-6.88 0-8.6.46a2.78 2.78 0 0 0-1.94 2A29 29 0 0 0 1 11.75a29 29 0 0 0 .46 5.33 2.78 2.78 0 0 0 1.94 2c1.72.46 8.6.46 8.6.46s6.88 0 8.6-.46a2.78 2.78 0 0 0 1.94-2 29 29 0 0 0 .46-5.33 29 29 0 0 0-.46-5.33z"></path><polygon points="9.75 15.02 15.5 11.75 9.75 8.48 9.75 15.02"></polygon></svg></a>
                    <a href="https://www.instagram.com/c4znu" target="_blank" rel="noreferrer" className={`transition-colors ${theme === 'dark' ? 'text-zinc-400 hover:text-white' : 'text-zinc-500 hover:text-zinc-900'}`}><svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg></a>
                    <a href="https://www.facebook.com/c4znu/" target="_blank" rel="noreferrer" className={`transition-colors ${theme === 'dark' ? 'text-zinc-400 hover:text-white' : 'text-zinc-500 hover:text-zinc-900'}`}><svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"></path></svg></a>
                  </div>
                </div>
            ...
            <div className="md:col-span-5 flex flex-col gap-5 w-full">
              ...
              {/* EXPERIENCE TIMELINE */}
              <div className={`border rounded-xl p-5 md:p-6 ${theme === 'dark' ? 'bg-zinc-950 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                <h2 className={`text-base font-extrabold tracking-tight mb-6 ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>Experience</h2>
                ...
                <div className="relative ml-2">
                  ...
                  <div className="flex flex-col gap-6">
                    ...
                    {/* Item 2 */}
                    <div className="relative pl-5 flex justify-between items-start group">
                      <div className={`absolute left-0 top-1.25 w-1.75 h-1.75 border z-10 transition-transform group-hover:scale-150 ${theme === 'dark' ? 'border-zinc-600 bg-zinc-950 group-hover:border-white' : 'border-zinc-300 bg-white group-hover:border-zinc-900'}`}></div>
                      <div className="pr-2">
              ...
              {/* EDUCATION */}
              <div className={`border rounded-xl p-5 md:p-6 ${theme === 'dark' ? 'bg-zinc-950 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                <h2 className={`text-[1rem] font-extrabold tracking-tight mb-4 ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>Education</h2>
              ...
              <div id="gallery" className={`border rounded-xl p-5 md:p-6 ${theme === 'dark' ? 'bg-zinc-950 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                <div className="flex justify-between items-center mb-4">
                  <h2 className={`text-base font-extrabold tracking-tight ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>Gallery</h2>
                  <button onClick={() => navigateTo('gallery')} className={`bg-transparent border-none text-[0.7rem] font-bold cursor-pointer inline-flex items-center gap-1 transition-colors group ${theme === 'dark' ? 'text-zinc-400 hover:text-white' : 'text-zinc-500 hover:text-zinc-900'}`}>
                    View All
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {galleryImages.slice(0, 6).map((image, idx) => (
                    <div 
                      key={idx} 
                      className={`flex aspect-square rounded-md overflow-hidden hover:opacity-80 transition-opacity cursor-pointer border ${theme === 'dark' ? 'border-zinc-800' : 'border-zinc-200'}`}
                      onClick={() => openModal(image.src)}
                    >
                      <img src={image.src} alt={image.alt} className="w-full h-full object-cover" />
                    </div>
